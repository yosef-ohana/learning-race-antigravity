        ...
        {hasOptions ? (
          <div className="options-grid">
            {question.options.map((opt, i) => (
              <button key={i} className="option-btn hebrew-text" onClick={() => onSubmitAnswer(opt.id)}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center' }}>
            <input 
              type="text" 
              className="input glow-box-blue hebrew-text" 
              ...
              autoFocus 
            />
            <button type="submit" className="option-btn hebrew-text" style={{ width: '100%', marginTop: '1rem' }}>שלח תשובה</button>
          </form>
        )}
      </div>
    </div>
