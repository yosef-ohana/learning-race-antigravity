...
const CarIcon = ({ color = '#bc13fe', width = 60, height = 30, style = {} }) => {
  ...
  const glowId = `neon-glow-${cleanColor}-${uniqueId}`;
  const gradId = `body-grad-${cleanColor}-${uniqueId}`;
  const metalGradId = `metal-grad-${uniqueId}`;
  const windowGradId = `window-grad-${uniqueId}`;
  ...
  return (
    ...
    >
      <defs>
        ...
        {/* Cyberpunk Window Glass Gradient */}
      ...
      {/* Main Car Body Silhouette - Lamborghini style */}
      <path 
      ...
      {/* Cyberpunk Cabin Window */}
      <path d="M 64 27 L 76 26 L 100 37 L 85 39 L 60 39 Z" fill={`url(#${windowGradId})`} stroke={color} strokeWidth="1" strokeLinejoin="round" />
      <path d="M 45 38 L 57 28 L 62 27 L 57 39 Z" fill={`url(#${windowGradId})`} stroke={color} strokeWidth="0.8" strokeLinejoin="round" />
      <path d="M 66 28 L 75 27 L 90 34" fill="none" stroke="#ffffff" strokeWidth="1" opacity="0.4" />

      {/* Cyber/Tech lines on body */}
      ...
      <circle cx="14" cy="56" r="2" fill="#111" stroke={`url(#${metalGradId})`} strokeWidth="1" />
      ...
      <g transform="translate(33, 55)">
        ...
        <circle cx="0" cy="0" r="11" fill="none" stroke={`url(#${metalGradId})`} strokeWidth="0.5" />
        ...
        <g stroke={`url(#${metalGradId})`} strokeWidth="0.8" opacity="0.6">
